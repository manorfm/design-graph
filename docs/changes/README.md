...
| [C35](C35-media-query-css-scoping/) | Parsing: `@media` corrompe a resolução de classe CSS | ✅ Done | T77–T78 | Crítico — `extract_css_rules`/`extract_tag_pseudo_rules` absorviam regras de dentro de `@media` como se fossem incondicionais, sobrescrevendo o valor default real (evidência: `.page-title` em `toToggle v2.2.html` perdia 2 de 3 propriedades e retornava o valor de viewport ≤600px como se fosse o padrão). T77 corrige a corrupção; T78 expõe as regras responsivas separadamente — nova coluna `media` em `Style`, seção "Estilos responsivos" em `get_component_spec` — verificado ponta a ponta contra o grafo reconstruído de `toToggle v2.2.html` (835→869 Styles, 4 condições `@media` do componente `AppList` renderizadas sem se misturar ao default) |
