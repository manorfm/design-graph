...
| [C42](C42-semantic-chrome-tags-as-sections/) | Extraction (feature): `<aside>`/`<nav>`/`<header>`/`<footer>` viram seção + nome por `className` | ✅ Done | T97 | Alto — pergunta direta do usuário sobre C38 ("Section1"/"Section2" em `App`, como o MCP acha "a sidebar" se pedirem pra atualizar ela?): investigação revelou que não era só nome — a sidebar real do `toToggle` (`<aside className={"sidebar"+...}>`) nunca virava seção nenhuma, porque o fallback estrutural só reconhecia `<div>`. T97 adiciona `<aside>`/`<nav>`/`<header>`/`<footer>` como candidatos (sem exigir padding próprio, ao contrário de `<div>`) e passa a nomear pelo `className` da própria tag raiz (tolerante a `className={"literal"+expr}`) em vez do primeiro texto visível — capacidade nova, não correção de um comportamento que já existia (feature, não fix). `<main>`/`<section>` marcados fora de escopo por ora. Verificado contra `App` no `toToggle v2.6` real: `Sidebar` (novo) e `Topbar` (antes "Menu") confirmados; `get_section(screen="App", section="sidebar")` passa a devolver a seção real |

> C23 nasceu de uma investigação real de sessão de agente, não de análise de
> código a priori — mesmo padrão de origem do C35–C42. C34 é uma segunda
...
> `@media` não têm escape hatch sem corte, ao contrário dos demais"). C42 é
> diferente dos demais: nasceu da mesma conversa, mas é capacidade nova
> (detectar tag semântica como seção), não correção de bug — feature, não fix.
