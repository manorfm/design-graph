...
_AGENT_INSTRUCTIONS = (
    ...
    "passing doc= on every call. This selection lives on the MCP connection, "
    "not the task, so it resets on a connection restart (e.g. '/mcp' "
    "reconnect) even mid-task — if a later call unexpectedly reports "
    "multiple prototypes loaded again, call set_prototype once more instead "
    "of assuming the earlier selection still holds. Prefer get_screen_full to reconstruct a "
